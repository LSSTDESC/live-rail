...
_SANDBOX_CATALOG_YAML = (
    Path(os.path.dirname(live_rail.__file__).replace("src/live_rail", "nb")) / "sandbox_catalogs.yaml"
)
...
class PzdcSetup(SetupProfile):
    ...
    def download(self) -> None:
        ...
        marker = _BASE_DIR / _MODEL_DIR / "flagship_gold_roman_1yr" / "model_inform_knn.pkl"
        if not marker.exists():
            print(f"Extracting to {_BASE_DIR} ...")
            result = subprocess.run(["tar", "zxvf", tar_file, "-C", str(_BASE_DIR)], check=False)
        ...
        for ver in ["10yr", "1yr"]:
            ...
            for cat in ["roman", "rubin"]:
                ...
                rail_svc_utils.safe_insert_dataset(
                    dataset_name, dataset_file, cat, n_objects=40000, is_collection=False, validate_file=False
                )

            matched_name = f"sandbox_roman_plus_rubin_{ver}"
            matched_tag = "roman_plus_rubin"
            matched_file = str(_TEST_DATA_DIR / f"sandbox_test_{matched_tag}_{ver}.hdf5")
        ...
        for algo_name, algo_class in ALGOS.items():
            ...
            for cat in ["roman", "rubin", "roman_plus_rubin"]:
                for ver in ["10yr", "1yr"]:
                    ...
                    model_file = str(
                        _MODEL_DIR / f"flagship_gold_{cat}_{ver}" / f"model_inform_{algo_name}.pkl"
                    )
                    qp_file = str(
                        _ESTIMATES_DIR / f"flagship_gold_{cat}_{ver}" / f"output_estimate_{algo_name}.hdf5"
                    )
                    ...
                    rail_svc_utils.safe_insert_estimates(
                        estimates_name, qp_file, estimator_name, dataset_name,
                        n_objects=40000, validate_file=False,
                    )
