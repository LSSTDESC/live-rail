...
class RailSvcCatalogWrapper(CatalogWrapper):
    ...
    def get_data(self, row: int) -> tuple[np.ndarray, np.ndarray, dict[str, qp.Ensemble], float|None, dict[str, np.ndarray]]:
        ...
        return np.squeeze(mag_vals), np.squeeze(mag_err_vals), estimates, true_redshift, input_data
        
    

class RailSvcSimpleMultiCatalogWrapper(RailSvcCatalogWrapper, MultiCatalogWrapper):

    def __init__(self, dataset_id: int) -> None:

        matched_wrapper = RailSvcCatalogWrapper(dataset_id)
        ...
        for dataset_assoc_ in dataset_assocs:
            ...
            component_wrappers[component_dataset.name] = RailSvcCatalogWrapper(component_dataset.id_)
        MultiCatalogWrapper.__init__(self, matched_wrapper, component_wrappers)
        RailSvcCatalogWrapper.__init__(self, dataset_id)
