...
from rail_svc import local_sync, remote_sync, models

from .object_wrapper import ObjectWrapper, CatalogWrapper, MultiCatalogWrapper
...
class RailSvcLocalCatalogWrapper(CatalogWrapper):
    ...
    def __init__(self, dataset_id: int) -> None:
        ...
        self._wrapped_estimators = {wrapper_estimator.estim_name:wrapper_estimator for wrapper_estimator in the_wrapped_estimators}
    ...
    def get_data(self, row: int) -> tuple[np.ndarray, np.ndarray, dict[str, qp.Ensemble], float|None, dict[str, np.ndarray]]:
        ...
        mag_vals = np.array([input_data[self._mag_column_map.get(band_name_)][row] for band_name_ in self._band_names])
        mag_err_vals = np.array([input_data[self._mag_err_column_map.get(band_name_)][row] for band_name_ in self._band_names])
...
class RailSvcRemoteCatalogWrapper(CatalogWrapper):
    ...
    def __init__(self, dataset_id: int) -> None:
        ...
        self._catalog_band_assocs = remote_sync.catalog_band_assoc().find_by(catalog_tag_id=self._dataset.catalog_tag_id)
        self._bands = [remote_sync.band().get_row(catalog_band_assoc_.band_id) for catalog_band_assoc_ in self._catalog_band_assocs]
        self._band_names = [band_.name for band_ in self._bands]
        self._band_midpoints = np.array([self.calc_midpoint(band_) for band_ in self._bands])
        self._estimate_names = [estimate_.name for estimate_ in self._estimates]
