...
def build_data_app(cat_data, pz_data):

    eval_grid = np.linspace(0, 3, 301)
    n_rows = pz_data.npdf
    true_redshifts = cat_data['redshift']
    pz_estimates = pz_data.ancil['zmode']
    pdfs = pz_data.pdf(eval_grid)
    ...
    # Define the layout
    app.layout = html.Div([
        html.H1("2D Array Interactive Plotter", 
    ...
    def update_plot(selected_row):
        ...
        fig.update_layout(
            title=f'Plot of Row {selected_row}',
            xaxis_title='z',
            yaxis_title='p(z)',
            hovermode='x unified',
        ...
        stats = html.Div([
            html.P(f"Mean: {np.mean(row_data):.4f}"),
            html.P(f"Std Dev: {np.std(row_data):.4f}"),
            html.P(f"Min: {np.min(row_data):.4f} (at index {np.argmin(row_data)})"),
            html.P(f"Max: {np.max(row_data):.4f} (at index {np.argmax(row_data)})"),
            html.P(f"Range: {np.ptp(row_data):.4f}"),
        ])
