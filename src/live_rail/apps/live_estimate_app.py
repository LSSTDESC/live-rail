...
class MultiCatalogRedshiftVisualizer:
    ...
    def _create_spectrum_panel(self) -> html.Div:
        ...
        controls = html.Div([
            ...
            html.Label("Magnitude Adjustments:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
    ...
    def _create_redshift_controls(self) -> html.Div:
        ...
        return html.Div([
            html.Div([
                html.Label("Error Scale:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                dcc.Slider(
            ...
            html.Hr(style={'margin': '10px 0'}),
            html.Div([
                html.Label("Individual Catalog Estimators:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                html.Div(id='catalog-estimator-checklists-container')
            ]),
            html.Hr(style={'margin': '10px 0'}),
            html.Div([
                ...
                dcc.Checklist(
                    ...
                    options=[
                        {'label': 'Show ensemble families (estimate_many_redshifts)', 'value': 'show_families'}
                    ],
                    value=[],
                    inline=False,
                    style={'marginBottom': '10px'}
                ),
                html.Label("Number of samples for families:", style={'fontWeight': 'bold', 'marginBottom': '5px', 'fontSize': '12px'}),
                dcc.Slider(
                    id='n-samples-slider',
                    min=5,
                    max=100,
    ...
    def setup_callbacks(self):
        ...
        @self.app.callback(
            ...
            [Input('current-object-idx', 'data'),
             ...
             Input({'type': 'magnitude-slider', 'band': ALL, 'catalog': ALL}, 'value')],
            [State({'type': 'magnitude-slider', 'band': ALL, 'catalog': ALL}, 'id'),
        ...
        def update_magnitude_sliders(
            ...
            # Reset adjustments if reset button clicked or object changed
            if trigger_id in ['reset-magnitudes-button', 'current-object-idx'] or current_adjustments is None:
                current_adjustments = {}
            ...
            if trigger_id and 'magnitude-slider' in trigger_id and slider_ids:
                for slider_id, value in zip(slider_ids, slider_values):
                    key = f"{slider_id['catalog']}_{slider_id['band']}"
            ...
            try:
                ...
                for cat_name in self.catalog_names:
                    ...
                    band_names = catalog_obj.get_band_names()
                    mags = catalog_obj.get_magnitudes()
                    
                    sliders.append(html.Div([
                        html.Label(f"{cat_name}:", style={'fontWeight': 'bold', 'fontSize': '12px', 'marginTop': '5px'}),
                    ]))
                    
                    for band, mag in zip(band_names, mags):
                        key = f"{cat_name}_{band}"
                        current_val = current_adjustments.get(key, 0.0)
                        
                        sliders.append(html.Div([
                            html.Label(f"{band}:", style={'fontSize': '11px', 'marginLeft': '10px'}),
                            dcc.Slider(
                                id={'type': 'magnitude-slider', 'catalog': cat_name, 'band': band},
                                min=-2.0,
                                max=2.0,
                                value=current_val,
                                step=0.05,
        ...
        def update_color_dropdowns(
            ...
            try:
                multi_obj = self.catalog.get_wrapper(idx)
                logger.info(f"Got wrapper for color dropdowns: {multi_obj}")
        ...
        @self.app.callback(
            [Output('combined-estimate-checklist', 'options'),
             ...
             Output('catalog-estimator-checklists-container', 'children')],
            Input('current-object-idx', 'data')
        )
        def update_redshift_checklists(
            idx: Optional[int]
        ) -> Tuple[List[Dict], List[str], List[html.Div]]:
            """Update redshift estimator checklists for combined and individual catalogs."""
            if idx is None:
                idx = 0
            ...
            try:
                ...
                # Create checklists for each catalog's estimators
                catalog_checklists = []
                for cat_name in self.catalog_names:
                    try:
                        ...
                        estimators = catalog_obj.get_wrapped_estimators()
                        ...
                        if estimator_names:
                            catalog_checklists.append(
                                html.Div([
                                    ...
                                    dcc.Checklist(
                                        id={'type': 'catalog-estimator-checklist', 'catalog': cat_name},
                                        options=[{'label': name, 'value': f'{cat_name}_{name}'} 
                                                for name in estimator_names],
                                        value=[],  # Start with none selected for on-the-fly estimation
                                        inline=False,
        ...
        def update_spectrum(
            ...
            """Update the spectrum plot for selected catalogs with magnitude adjustments."""
            if idx is None:
                idx = 0
            ...
            try:
                ...
                multi_obj = self.catalog.get_wrapper(idx)
                logger.info(f"Got wrapper for spectrum: {multi_obj}")
                ...
                for i, cat_name in enumerate(selected_catalogs):
                    try:
                        ...
                        for j, band in enumerate(band_names):
                            key = f"{cat_name}_{band}"
                            if key in mag_adjustments:
                                mags[j] += mag_adjustments[key]
        ...
        def update_color_color(
            ...
            """Update the color-color diagram with magnitude adjustments."""
            if idx is None:
                idx = 0
            ...
            try:
                ...
                multi_obj = self.catalog.get_wrapper(idx)
                logger.info(f"Got wrapper for color-color: {multi_obj}")
                ...
                mags = catalog_obj.get_magnitudes().copy()
                mag_errors = catalog_obj.get_magnitude_errors()
                band_names = catalog_obj.get_band_names()
                
                # Apply adjustments
                for j, band in enumerate(band_names):
                    key = f"{catalog_name}_{band}"
                    if key in mag_adjustments:
                        mags[j] += mag_adjustments[key]
        ...
        def update_redshift_plot(
            ...
            try:
                ...
                logger.info(f"Got wrapper for redshift: {multi_obj}")
                ...
                # Plot combined estimates (pre-computed)
                if combined_selected:
                    self._add_combined_estimates_to_plot(
                        fig, multi_obj, combined_selected, z_grid, idx
                    )
                
                # Plot individual catalog estimates (on-the-fly)
                if catalog_selected_list and catalog_ids:
                    self._add_catalog_estimates_to_plot_live(
                        fig, multi_obj, catalog_selected_list, catalog_ids, 
                        z_grid, idx, error_scale, mag_adjustments, 
                        show_families, n_samples
                    )
    ...
    def _add_combined_estimates_to_plot(
        ...
        try:
            logger.info(f"Adding combined estimates: {idx}")

            combined_estimates = multi_obj.get_redshift_estimates()
            combined_colors = px.colors.qualitative.Dark24

            logger.info(f"Combined estimates loop: {combined_selected}")
            
            for i, estimate_key in enumerate(combined_selected):
                try:
                    logger.debug(f"Adding combined estimate 1: {estimate_key}")
                    ...
                    logger.debug(f"Adding combined estimate 2: {estimate_name}")
                    ...
                    logger.debug(f"Adding combined estimate 3: {estimate_name}")
    ...
    def _add_catalog_estimates_to_plot_live(
        self,
        fig: go.Figure,
        multi_obj: Any,
        catalog_selected_list: List[List[str]],
        ...
        """
        Add individual catalog redshift estimates to the plot with on-the-fly calculation.
        ...
        catalog_selected_list : List[List[str]]
            Selected estimators for each catalog
        catalog_ids : List[Dict]
        ...
        try:
            ...
            logging.debug(f"Catalog IDs: {catalog_ids}")

            for catalog_selected, catalog_id in zip(catalog_selected_list, catalog_ids):
                logging.debug(f"Processing: {catalog_selected}, {catalog_id}")
                ...
                try:
                    ...
                    # Get base input data
                    base_input_data = catalog_obj._input_data.copy()
                    ...
                    for band in band_names:
                        ...
                        if key in mag_adjustments:
                            ...
                            mag_name_dict = catalog_obj._parent._mag_name_dict
                            if band in mag_name_dict:
                                mag_col = mag_name_dict[band]
                                base_input_data[mag_col] = base_input_data[mag_col] + mag_adjustments[key]
                    
                    # Apply error scale
                    mag_err_column_map = catalog_obj._parent._mag_err_column_map
                    for band in band_names:
                    ...
                    for estimate_key in catalog_selected:
                        logging.debug(f"Processing estimator: {estimate_key}")

                        try:
                            # Parse estimator name from key (format: catalog_estimatorname)
                            estimator_name = estimate_key.replace(f'{cat_name}_', '')
                            ...
                            try:
                                ensemble = catalog_obj.estimate_redshift(estimator_name)
                                pdf = np.squeeze(ensemble.pdf(z_grid))
                                ...
                                fig.add_trace(go.Scatter(
                                    ...
                                    y=pdf,
                                    mode='lines',
                                    name=f'{cat_name}: {estimator_name}',
                                ...
                                if mode_val is not None and mode_val > 0:
                                    ...
                                    fig.add_trace(go.Scatter(
                                        ...
                                        name=f'{cat_name} {estimator_name} mode',
                                        showlegend=False
                                    ))
                                
                                logger.debug(f"Added catalog estimate: {cat_name} - {estimator_name}")
                            except Exception as e:
                            ...
                            if show_families:
                                try:
                                    # Build catalog samples with error scaling
                                    from live_rail.wrappers.object_wrapper import ObjectWrapper
                                    ...
                                    ensemble_family = catalog_obj.estimate_many_redshifts(
                                        estimator_name, 
                                        samples_data
                                    )
                                    ...
                                    for sample_idx in range(n_samples):
                                        try:
                                            ...
                                            sample_ensemble = ensemble_family[sample_idx]
                                            sample_pdf = np.squeeze(sample_ensemble.pdf(z_grid))
                                            
                                            fig.add_trace(go.Scatter(
                                                x=z_grid,
                                                y=sample_pdf,
                                                mode='lines',
                                                name=f'{cat_name}: {estimator_name} family',
                                                line=dict(width=1, color=color, dash='dot'),
                                                opacity=0.3,
                                                showlegend=(sample_idx == 0),  # Only show legend for first trace
                                                legendgroup=f'{cat_name}_{estimator_name}_family'
                                            ))
                                        except Exception as e:
                                            logger.warning(f"Error plotting family member {sample_idx}: {e}")
                                            continue
                                    
                                    logger.debug(f"Added ensemble family for {cat_name} - {estimator_name}")
                                except Exception as e:
